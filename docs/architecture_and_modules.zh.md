...
- `selectedSkillId` 存于浏览器 local storage
...
- 以 `skills/<skill-id>/SKILL.md` 作为内置 skill 的源数据
- 将 markdown skill 文档解析为运行时 skill profile
- 组合基础策略、当前 skill、call purpose 与语言约束，生成最终 `sessionInstruction`
- 保持 skill 逻辑位于 prompt 组装层，不进入音频模块和传输协议代码
